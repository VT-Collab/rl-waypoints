...
class OAT(object):
    def __init__(self, state_dim, objs, wp_id, save_name, epochs, load_models=False):
        ...
        ## seed random numbers
        """ NOT included: best traj (init), xi0 (epoch), pose noise (epoch) """
        np.random.seed(2)
        self.reset_probs = np.random.rand(epochs)
        self.reset_model_nums = np.random.randint(10, size=epochs)
        self.reward_indices = np.random.randint(self.n_models, size=epochs)
        self.best_wps = np.random.rand(epochs, self.state_dim)
        self.gripper_noise_probs = np.random.rand(epochs)
        self.pose_noise_probs = np.random.rand(epochs)
    ...
    def traj_opt(self, i_episode, objs):
        ...
        self.reward_idx = self.reward_indices[i_episode-1]
        ...
        for idx in range(1, self.wp_id+1):
            ...
            if idx == self.wp_id and i_episode <= 50:
                # self.best_wp = 0.5*(np.random.rand(self.state_dim) - 0.5)
                self.best_wp = 0.5*(self.best_wps[i_episode-1, :] - 0.5)
                ...
                for t_idx in range(self.n_inits):
                    ...
                    if res.fun < min_cost:
                        min_cost = res.fun
                        self.best_wp = res.x
                # if idx == self.wp_id and np.random.rand() < 0.5 and i_episode>100 and i_episode<150:
                if idx == self.wp_id and self.gripper_noise_probs[i_episode-1] < 0.5 and i_episode>100 and i_episode<150:
                    tqdm.write("NOISE ADDED TO GRIPPER")
                    self.best_wp[-1] *= -1
                # if idx == self.wp_id and np.random.rand() < 0.5 and i_episode>100 and i_episode<150:
                if idx == self.wp_id and self.pose_noise_probs[i_episode-1] < 0.5 and i_episode>100 and i_episode<150:
