...
def train(args):
    ...
    state_dim = len(obs['robot0_eef_pos']) + len(R.from_quat(obs['robot0_eef_quat']).as_euler('xyz', degrees=False)) +  len(objs)
    ...
    for i_episode in itertools.count(1):
        ...
        state = list(obs['robot0_eef_pos']) + list(R.from_quat(obs['robot0_eef_quat']).as_euler('xyz', degrees=False)) + list(objs)
        ...
        for timestep in range(1, max_ep_len+1):
            ...
            full_action = list(action) # list(action[0:3]) + [0.]*3 + [action[3]]
