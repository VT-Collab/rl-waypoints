...
def train(args):
    ...
    state_dim = len(obs['robot0_eef_pos']) + len(Rotation.from_quat(obs['robot0_eef_quat']).as_euler('xyz', degrees=False)) + len(objs)
    ...
    for i_episode in itertools.count(1):
        ...
        for _ in range(num_waypoints):
            ...
            for timestep in range(50):
                ...
                error_angle = waypoint_normalized[3:6] - Rotation.from_quat(obs['robot0_eef_quat']).as_euler('xyz')[-1]
...
if __name__ == '__main__':
    ...
    p.add_argument('--num_eval', type=int, default=999)
    args = p.parse_args()
    train(args)
