...
class NutAssembly(SingleArmEnv):
    ...
    def reward(self, action=None):
        ...
        self._check_success()
        reward = np.sum(self.objects_on_pegs)*20

        # add in shaped rewards
        if self.reward_shaping:
            staged_rewards = self.staged_rewards()
            idx = np.argmax(abs(staged_rewards))
            reward += staged_rewards[idx]
    ...
    def staged_rewards(self):
        ...
        if active_nuts:
            ...
            home_quat = [0, 0, 0, 1]
            r = R.from_quat(home_quat)
            home_eul = r.as_euler('xyz', degrees=False)
            curr_quat = T.convert_quat(self.sim.data.body_xquat[self.obj_body_id[self.nut_type]], 'xyzw')
            r_curr = R.from_quat(curr_quat)
            curr_eul = r_curr.as_euler('xyz', degrees=False)

            diff_eul = np.linalg.norm(home_eul[:2] - curr_eul[:2])
        ...
        if active_nuts and r_grasp>0:
            ...
            nut_lifted = np.any(object_z_locs>z_target)
            r_lift = lift_mult if nut_lifted else 0.0
            # r_lift = (1 - np.tanh(15.0 * min(z_dists))) * (lift_mult)

        # hover reward for getting object above peg
        r_hover = 0.0
        if active_nuts and r_lift>0:
            ...
            for i, nut in enumerate(active_nuts):
                ...
                # r_hovers[i] = r_lift + (1 - np.tanh(100.0 * dist)) * (hover_mult - lift_mult)
                r_hovers[i] = (1 - np .tanh(5.0 * dist)) * (hover_mult)*2.5
    ...
    def _load_model(self):
        ...
        if self.placement_initializer is None:
            ...
            for nut_name, default_y_range in zip(nut_names, ([-0.11, 0.11], [-0.11, 0.11])):
