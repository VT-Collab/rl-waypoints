...
class PickPlace(SingleArmEnv):
    ...
    def reward(self, action=None):
        ...
        if self.reward_shaping:
            ...
            reward += staged_rewards[idx] # max(staged_rewards)
    ...
    def staged_rewards(self):
        ...
        if active_objs:
            ...
            curr_quat = T.convert_quat(self.sim.data.body_xquat[self.obj_body_id[self.obj_type]], 'xyzw')
            r_curr = R.from_quat(curr_quat)
            curr_eul = r_curr.as_euler('xyz', degrees=False)

            diff_eul = np.linalg.norm(home_eul - curr_eul)
        ...
        for idx, obj in enumerate(self.objects):
            ...
            self.placement_initializer.append_sampler(
                sampler=UniformRandomSampler(
                    name=f"{obj}CollisionObjectSampler",
                    mujoco_objects=obj,
                    x_range=[-bin_x_half/2, bin_x_half/2],
                    y_range=[-bin_y_half/2, bin_y_half/2],
                    # x_range=[0.1, 0.1] if idx == 0 else [-0.05, -0.05] if idx == 1 else [0.1, 0.1] if idx == 3 else [-0.05, -0.05],
                    # y_range=[-0.05, -0.05] if idx == 0 else [0.15, 0.15] if idx == 1 else [-0.05, -0.05] if idx == 2 else [0.15, 0.15],
                    rotation=0.0,
                    rotation_axis="z",
