...
# Agent
agent = Method(traj_dim=dim_high, state_dim=1)

# Memory
memory = MyMemory()

# Logger
run_name = 'runs/high_dim_' + datetime.datetime.now().strftime("%H-%M")
...
# Main loop
total_steps = 0
for i_episode in range(1, 151):

    # initialize variables
    episode_reward = 0
    
    # get traj
    traj = 2.0*(np.random.rand(dim_high)-0.5)
    ...
    f = interpolate.interp1d(np.linspace(0, 1, dim_high), traj)
...
pickle.dump(dataset, open("runs/rewards/high.pkl", "wb"))