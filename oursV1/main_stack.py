...
# Logger
run_name = 'runs/ours_stack_' + datetime.datetime.now().strftime("%H-%M")
...
for i_episode in range(1, 1001):
    ...
    traj = 0.5*(np.random.rand(16)-0.5)
    traj[3]*=2
    traj[7]*=2
    traj[11]*=2
    traj[15]*=2
    if i_episode > 40:
        traj = agent.traj_opt()
    traj_mat = np.reshape(traj, (4,4)) + robot_home
    traj_mat[:,3] *= 2.0
