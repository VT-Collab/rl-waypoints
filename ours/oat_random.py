...
#         # objs = np.concatenate((pos, [eul[-1]], [obs['nut_id']]))
#         objs = pos
#         # state_dim = 4
#     agent = OAT(state_dim=state_dim, objs=objs, wp_id=wp_id, save_name=save_name)
...
#         if np.random.rand()<0.05 and i_episode<450 and i_episode>1:
#             agent.reset_model(np.random.randint(10))
...
#         traj = traj_full[-state_dim:]
#         traj_mat = np.reshape(traj_full, (wp_id,state_dim))[:, :state_dim-1]
#         pos = obs['robot0_eef_pos']
#         # if args.env == 'NutAssembly':
#         #     r_robot = R.from_quat(obs['robot0_eef_quat'])
#         #     eul_robot = r_robot.as_euler('xyz', degrees=False)
#         #     pos = np.append(pos, eul_robot[-1])
#         #     traj_mat[:, -1] *= 5
...
#             # convert traj to actions
#             state = obs['robot0_eef_pos']
#             # quat_robot = obs['robot0_eef_quat']
#             # r_robot = R.from_quat(quat_robot)
#             # eul_robot = r_robot.as_euler('xyz', degrees=False)
#             g_state = obs['robot0_gripper_qpos']
#             widx = int(np.floor(timestep / (50)))
#             error = np.zeros(6)
#             error[:3] = traj_mat[widx, :3] - state
#             # if args.env == 'NutAssembly':
#             #     error[-1] = traj_mat[widx, 3] - eul_robot[2]
