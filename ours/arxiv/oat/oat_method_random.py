...
class OAT(object):
    def __init__(self, state_dim, objs, wp_id, save_name):
        ...
        for wp_id in range(1, self.wp_id):
            ...
            # critic = RNetwork(wp_id*self.state_dim + len(objs), hidden_dim=self.hidden_size).to(device=self.device)
            # critic.load_state_dict(torch.load(save_dir + '/model_best' + str(wp_id)))
            # self.learned_models.append(critic)


        self.best_traj = 0.5*(np.random.rand(self.state_dim*self.wp_id) - 0.5)*self.wp_id
        self.best_reward = -np.inf
        self.lincon = LinearConstraint(np.eye(self.state_dim), -1.0, 1.0)
    ...
    def traj_opt(self, i_episode, objs):
        ...
        for idx in range(1, self.wp_id+1):
            ...
            if idx == self.wp_id and i_episode <= 75:
                self.best_wp = 0.5*(np.random.rand(self.state_dim) - 0.5)*self.wp_id
                ...
                for t_idx in range(self.n_inits):
                    ...
                    if res.fun < min_cost:
                        min_cost = res.fun
                        self.best_wp = res.x
                if idx == self.wp_id and np.random.rand() < 50/i_episode and i_episode<250:
